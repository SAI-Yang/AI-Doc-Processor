...
_BUILTIN_TEMPLATES: dict[str, Template] = {
    ...
    "academic_polish": Template(
        ...
        system_prompt=(
            ...
            "4. 【关键】严格禁止添加原文没有的解释、说明或补充内容\n"
            "5. 【关键】严格保留原文中的空格、缩进和特殊排版格式，包括封面字间空格（如实 验 报 告中间的空格）\n"
            "6. 封面格式、页眉页脚、段落间距等排版信息完全保持不变\n"
            "7. 不要给文章分段、加标题或重新组织结构\n"
            '8. 不要输出"修改说明"或任何额外文字\n'
            "9. 只输出润色后的原文，不增不减\n\n"
            "润色方向：\n"
            "- 修正语法错误和不通顺的表达\n"
    ...
    "general_polish": Template(
        ...
        system_prompt=(
            ...
            "5. 适当精简冗余表达\n\n"
            "【重要约束】\n"
            "- 严格保留原文中的空格、缩进和特殊排版格式\n"
            "- 严格禁止添加原文没有的解释、说明或补充内容\n"
            "- 封面格式、段落间距等排版信息完全保持不变\n"
            "- 保持原文风格和语气，不改变原意\n"
            "- 如果原文无明显问题，可保持基本不变\n"
            "- 只输出润色后的文本。"
        ),
